...
// BufferTimeConfig is the configuration type for BufferTime.
...
func (conf BufferTimeConfig) MakeFunc() OperatorFunc {
	return MakeFunc(bufferTimeOperator(conf).Call)
}

type bufferTimeOperator BufferTimeConfig
...
//
// BufferTime collects values from the past as an slice, and emits those slices
